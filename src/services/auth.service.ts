...
export async function resetPassword(
  ...
  return await processTransaction(async function (client) {
    ...
    const findCodeQuery = `
    SELECT 
    u.id, u.password as hashed_password, p.expires_at
    FROM "user" u
  ...
  `;
    ...
    const { id: userId, hashed_password, expires_at } = codeRes[0];
    ...
    const isSame = await bcrypt.compare(newPassword, hashed_password);
    if (isSame) {
    ...
    await client.query(`UPDATE "user" SET 
      password = $1 WHERE id = $2`, [hashedPassword, userId]);
